...
def main():
    ...
    for name, batch, queries, heads, channels, sizes in CASES:
        ...
        value = torch.randn(batch, sum(h * w for h, w in sizes), heads, channels, requires_grad=True)
        ...
        for mode in ("forward", "forward_backward"):
            for backend, base_fn in (("cpp", extension), ("pytorch_static_shapes", reference)):
                for compile in (False, True):
                    ...
                    try:
                        fn = torch.compile(base_fn, backend="inductor", fullgraph=True) if compile else base_fn
                        ...
                        row.update(median_ms=measurement.median * 1000,
                                   iqr_ms=measurement.iqr * 1000)
                    except Exception as exc:
                        row["error"] = f"{type(exc).__name__}: {exc}"
                        print(row["error"], file=sys.stderr, flush=True)
                    rows.append(row)
    print(json.dumps(dict(torch=torch.__version__, python=platform.python_version(),
                          platform=platform.platform(), device="cpu", dtype="float32",
                          threads=args.threads, min_run_time_seconds=args.min_run_time,
                          compiler="inductor", fullgraph=True,
