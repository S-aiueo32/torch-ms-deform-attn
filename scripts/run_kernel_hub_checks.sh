...
msda_torch=2.10.0
msda_vision=0.25.0
if [[ "${2:-full}" == nix-full ]]; then
    msda_torch=2.11.0
    msda_vision=0.26.0
    test -f "$msda_source/kernel-hub-prepared/NIX_BUILD.json"
fi
python -m pip install --no-cache-dir "torch==$msda_torch" "torchvision==$msda_vision" \
...
if [[ "${2:-full}" == nix-full ]]; then
    cp "$msda_work/candidate/NIX_BUILD.json" "$msda_work/candidate/nix-UPSTREAM.json" "$msda_output/"
else
