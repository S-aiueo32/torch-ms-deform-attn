...
IMAGE = ("pytorch/pytorch:2.5.1-cuda12.4-cudnn9-devel@sha256:"
         "14611869895df612b7b07227d5925f30ec3cd6673bad58ce3d84ed107950e014")
GPU_IDS = {"A5000": "NVIDIA RTX A5000", "L4": "NVIDIA L4",
...
class RunpodAPI:
    ...
    def request(self, method, path, payload=None):
        ...
        request = urllib.request.Request(url, data=body, method=method, headers={
            ...
            "User-Agent": "torch-deform-attn-ci",
        })
    ...
    def quote(self, gpu, cap):
        query = urllib.parse.urlencode({"include": "AVAILABILITY", "product": "POD", "count": 1,
                                       "cloud": "SECURE", "minCudaVersion": "12.4"})
        result = self.request("GET", "/catalog/gpus/" + urllib.parse.quote(GPU_IDS[gpu], safe="") + "?" + query)
...
def identity(args):
    return {"CI_REPOSITORY": args.repository, "CI_REPOSITORY_ID": str(args.repository_id),
            "CI_RUN_ID": str(args.run_id), "CI_ATTEMPT": str(args.attempt)}
...
def delete_owned(api, pod_id, owner, created_at=None):
    for attempt in range(6):
        ...
        if not owned_pod(pod, owner, created_at):
            raise ControllerError("Refusing to delete a Pod whose ownership metadata does not match")
        try:
...
def cleanup(api, args):
    ...
    print(f"Cleanup complete for {pod_name(owner)} ({len(ids)} Pod identifiers checked)", flush=True)
...
def recover_create(api, owner, created_at):
    for attempt in range(7):
        ...
        if attempt < 6:
            time.sleep(5)
    raise ControllerError("Pod creation response was lost and recovery found no Pod; cleanup will still run")
...
def ssh_command(directory, name, pod):
    ...
    return ["ssh", "-F", "/dev/null", "-i", str(directory / "client"),
            "-o", "BatchMode=yes", "-o", "IdentitiesOnly=yes", "-o", "StrictHostKeyChecking=yes",
            "-o", "UserKnownHostsFile=" + str(directory / "known_hosts"),
            "-o", "HostKeyAlias=" + name, "-o", "ConnectTimeout=10",
            "-o", "ServerAliveInterval=15", "-o", "ServerAliveCountMax=3",
            "-p", str(port), "ci@" + address]
...
def wait_for_ssh(api, args, state, deadline):
    ...
    while time.monotonic() < ssh_deadline:
        ...
        try:
            ...
            continue
        try:
            result = subprocess.run(command + ["test -d /workspace/ci/source && test -d /workspace/ci/results"],
...
def stream_command(command, output, deadline, tee=False, limit=None, merge_stderr=True):
    process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT if merge_stderr else None,
...
def extract_artifacts(archive, destination):
    ...
    with tarfile.open(archive, mode="r:") as bundle:
        ...
        for member in members:
            ...
            if (not member.isfile() or len(path.parts) != 1 or path.name != name
...
def collect_artifacts(ssh, directory):
    command = ("cd /workspace/ci/results && find . -maxdepth 1 -type f "
    ...
    with archive.open("wb") as output:
        result = stream_command(ssh + ["bash -o pipefail -c " + shlex.quote(command)], output,
                                time.monotonic() + 120, limit=MAX_ARTIFACT_BYTES, merge_stderr=False)
    if result:
...
def run(api, args):
    ...
    if read_state(args.state_dir) is not None or any(args.state_dir.iterdir()):
        raise ControllerError("Use a new empty state directory; run cleanup for a previous creation intent")
    ...
    subprocess.run(["git", "-C", str(args.source), "archive", "--format=tar", "--output=" + str(archive), "HEAD"],
                   check=True, env=child_environment(), timeout=60)
    created = utc_now().isoformat()
    state = {"owner": owner, "created_at": created, "phase": "intent", "pod_id": None}
    write_state(args.state_dir, state)
    payload = {"name": pod_name(owner), "image": IMAGE, "disk": 40,
               "gpu": {"id": GPU_IDS[args.gpu], "count": 1, "minRamPerGpu": 16,
                       "minVcpuCountPerGpu": 4, "minCudaVersion": "12.4"},
               "cloud": "SECURE", "ports": ["22/tcp"], "startSsh": False, "startJupyter": False,
               "args": "/bin/bash -c " + shlex.quote(bootstrap.read_text()),
               "env": dict(owner, CI_CREATED_AT=created, CI_SSH_PUBLIC_KEY=public,
                           CI_SSH_HOST_KEY_B64=host_private, CI_MAX_SECONDS=str(args.timeout_minutes * 60))}
    ssh = None
    error = None
    try:
        ...
        with archive.open("rb") as source:
            subprocess.run(ssh + ["tar -xf - -C /workspace/ci/source"], stdin=source, check=True,
        ...
        if ssh is not None:
            try:
                ...
                print("Artifact collection failed; cleanup will continue", file=sys.stderr, flush=True)
                if error is None:
                    error = caught
        try:
            cleanup(api, args)
        except Exception as caught:
            print("Pod cleanup failed; the separate cleanup job must retry", file=sys.stderr, flush=True)
...
def reap(api, args):
    ...
    failures = 0
    for item in api.list_pods():
        if not isinstance(item, dict) or not str(item.get("name", "")).startswith(f"tda-gha-{args.repository_id}-"):
            continue
        try:
            pod = api.get_pod(item.get("id", ""))
        except APIError as error:
        ...
        if not isinstance(metadata, dict):
            continue
        owner = {key: metadata.get(key) for key in ("CI_REPOSITORY", "CI_REPOSITORY_ID", "CI_RUN_ID", "CI_ATTEMPT")}
        if (owner["CI_REPOSITORY"] != args.repository or owner["CI_REPOSITORY_ID"] != str(args.repository_id)
                or not all(re.fullmatch(r"[1-9][0-9]*", str(owner[key] or "")) for key in ("CI_RUN_ID", "CI_ATTEMPT"))
...
def parse_args(argv=None):
    ...
    run_parser = commands.add_parser("run", help="Rent one GPU, test, collect artifacts, and delete it")
    cleanup_parser = commands.add_parser("cleanup", help="Delete Pods belonging to one workflow attempt")
    reap_parser = commands.add_parser("reap", help="Delete this repository's owned Pods older than two hours")
    check_parser = commands.add_parser("check", help="Read-only API authentication and GPU-price check")
    ...
    for subparser in (run_parser, check_parser):
        subparser.add_argument("--gpu", choices=list(GPU_IDS) + list(GPU_IDS.values()), default="A5000")
    ...
    if hasattr(args, "repository") and not re.fullmatch(r"[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+", args.repository):
    ...
    if args.command == "run" and (args.state_dir == args.output_dir
