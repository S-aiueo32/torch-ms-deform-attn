...
class ControllerTest(unittest.TestCase):
    def setUp(self):
        ...
        self.args = argparse.Namespace(
            repository="example/torch-deform-attn", repository_id="123",
        ...
        self.pod = {
            "id": "ownedpod", "name": ci.pod_name(self.owner),
            "env": dict(self.owner, CI_CREATED_AT=self.created), "cost": 0.27,
        }
        self.sleep = mock.patch.object(ci.time, "sleep").start()
        self.addCleanup(mock.patch.stopall)

    def save_state(self, pod_id="ownedpod"):
        ci.write_state(self.args.state_dir, {
            "owner": self.owner, "created_at": self.created,
            "pod_id": pod_id, "phase": "created",
        })
    ...
    def prepare_run(self):
        ...
        def fake_subprocess(command, **kwargs):
            if command[0] == "git":
                target = next(value.split("=", 1)[1] for value in command if value.startswith("--output="))
                Path(target).write_bytes(b"fake source archive")
            return subprocess.CompletedProcess(command, 0)

        mock.patch.object(ci, "generate_keys", return_value=("ssh-ed25519 AAAA", "host-key-b64")).start()
    ...
    def test_lost_creation_response_is_recovered_without_a_second_post(self):
        ...
        with mock.patch.object(ci, "wait_for_ssh", side_effect=ci.ControllerError("SSH unavailable")):
    ...
    def test_test_and_artifact_failures_still_delete_pod(self):
        ...
        with mock.patch.object(ci, "wait_for_ssh", return_value=["ssh"]), \
                ...
                mock.patch.object(ci, "collect_artifacts", side_effect=ci.ControllerError("download failed")):
...
class PriceAndTransportTest(unittest.TestCase):
    ...
    def quote_response(self, **overrides):
        value = {"id": "NVIDIA RTX A5000", "secure": True,
                 "availability": "HIGH", "price": {"secure": 0.27}}
    ...
    def test_quote_rejects_unknown_capacity_wrong_gpu_and_excess_price(self):
        for response in (
            ...
            with self.subTest(response=response), mock.patch.object(self.api, "request", return_value=response):
                with self.assertRaises(ci.ControllerError):
    ...
    def test_http_failure_redacts_secret_and_does_not_retry_post(self):
        failure = urllib.error.HTTPError("https://api.runpod.io/v2/pods", 503,
                                         "example-secret-key", {}, io.BytesIO(b"private response"))
    ...
    def test_api_key_is_removed_from_child_environment(self):
        with mock.patch.dict(ci.os.environ, {"RUNPOD_API_KEY": "example-secret-key", "PATH": "/usr/bin"}):
    ...
    def test_ssh_uses_pinned_host_key_and_ci_user(self):
        command = ci.ssh_command(Path("/tmp/keys"), "unique-pod-name", {
            "ssh": {"direct": {"host": "203.0.113.10", "port": 34567,
                                 "username": "root", "command": "untrusted command"}},
        })
        ...
        for host, port in (("-oProxyCommand=touch /tmp/unsafe", 22), ("203.0.113.10", 65536)):
            with self.subTest(host=host, port=port):
                with self.assertRaises((ci.ControllerError, ValueError)):
                    ci.ssh_command(Path("/tmp/keys"), "pod", {"ssh": {"direct": {"host": host, "port": port}}})
    ...
    def test_generated_known_hosts_uses_only_generated_host_key(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual((directory / "known_hosts").read_text(), "known-pod ssh-ed25519 public-host\n")
...
class ArtifactTest(unittest.TestCase):
    ...
    def test_rejects_paths_links_devices_and_executables(self):
        for name, kind in (
            ("../outside.log", tarfile.REGTYPE), ("/absolute.log", tarfile.REGTYPE),
            ("nested/output.log", tarfile.REGTYPE), ("link.log", tarfile.SYMTYPE),
            ("hardlink.log", tarfile.LNKTYPE), ("device.log", tarfile.CHRTYPE),
            ("script.sh", tarfile.REGTYPE), ("directory.log", tarfile.DIRTYPE),
        ):
