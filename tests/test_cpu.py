...
"""
            ...
            for noncontiguous in (False, True):
                with self.subTest(dtype=dtype, noncontiguous=noncontiguous):
                    ...
                    actual = MSDeformAttnFunction.apply(
                        value, shapes, starts, locations, weights, 2
                    )
    ...
    def test_reject_invalid_gradient(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "grad_output shape"):
            MSDA.ms_deform_attn_backward(
                ...
                locations,
                weights,
                torch.zeros(3, 2, 5, dtype=value.dtype),
                64,
            )
    ...
    def test_gradcheck(self):
        ...
        self.assertTrue(
            gradcheck(MSDeformAttnFunction.apply, (value, shapes, starts, locations, weights, 64))
        )
    ...
    def test_level_start_index(self):
        ...
        torch.testing.assert_close(actual, expected)
        for a, e in zip(
            torch.autograd.grad(actual.sum(), (value, locations, weights)),
    ...
    def test_empty_queries(self):
        ...
        output = MSDeformAttnFunction.apply(
            value, shapes, starts, locations[:, :0], weights[:, :0], 64
        )
