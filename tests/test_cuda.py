...
class CUDAAttentionTest(SamplingCases, unittest.TestCase):
    ...
    def inputs(self, dtype=torch.double, channels=2, device="cuda", batch=2):
        ...
        locations = (
            torch.rand(batch, 2, 2, 2, 2, 2, dtype=dtype, device=device) * 1.6 - 0.3
        ).requires_grad_()
    ...
    def test_compile_dynamic(self):
        ...
        for batch in (3, 5):
            ...
            torch.testing.assert_close(actual, expected)
            for a, e in zip(
                torch.autograd.grad(actual.sum(), (args[0], args[3], args[4])),
...
@unittest.skipUnless(
    torch.cuda.is_available() and not _C.with_cuda, "Requires GPU and CPU-only build"
)
